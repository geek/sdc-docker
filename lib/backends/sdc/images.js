 ...
 *
 * @param digestChain {Array} Digest ancestor chain.
...
function dockerImageJsonToModel(imageJson, opts) {
    ...
    assert.arrayOfString(opts.digestChain, 'opts.digestChain');
    ...
    if (imageJson.docker_version) {
        modelObj.docker_version = imageJson.docker_version;
    }
    if (imageJson.history) {
        modelObj.history = imageJson.history;
    }
    if (imageJson.parent) {
        modelObj.parent = imageJson.parent;
    }
    if (imageJson.rootfs) {
        modelObj.rootfs = imageJson.rootfs;
    }
...
function getImageHistory(opts, callback) {
    ...
    if (opts.img.history) {
        history = opts.img.history.reverse().map(function (h) {
            return {
                Id: '<missing>',
                Comment: h.comment || '',
                Created: Math.floor((new Date(h.created)).getTime() / 1000),
    ...
    getImageAncestry(opts, function (err, ancestry) {
        ...
        history = ancestry.map(function (img) {
            var createdBy = '';
...
function getScratchImage(req, callback) {
    ...
    var digestChain = ['sha256:' + imageJson.id];
    var imageUuid = imgmanifest.imgUuidFromDockerDigests(digestChain);
    ...
    function imageJsonToModel() {
        var modelOpts = {
            ...
            manifest_str: '{ scatchImage: true }',  // A fake manifest.
            manifest_digest: '',
...
function deleteImage(opts, callback) {
    ...
    vasync.pipeline({arg: {}, funcs: [
        ...
        untagHeads,
        deleteImgs,
        deleteUnusedImgapiOriginImages
    ]}, function (err) {
            ...
            func: function untagOne(imgTag, nextImgTag) {
                ...
                ImageTag.del(app, log, imgTag, function (err) {
                    if (err) {
                        ...
                        if (isV2Image) {
                            changes.push({
                                Untagged: imgTag.repo + '@'
                                + ctx.img.manifest_digest });
                        }
                        nextImgTag();
                    }
                });
        ...
        function deleteOneImg(img, nextImg) {
            ...
            if (img.refcount > 1) {
                ...
                Image.del(app, log, img, function (delErr) {
                    ...
                    changes.push({
                        Deleted: (img.manifest_digest || img.docker_id) });
    ...
    function deleteUnusedImgapiOriginImages(ctx, cb) {
        if (!isV2Image) {
            cb();
            return;
        }

        // For v2 docker images, we need to check the images in the IMGAPI
        // origin chain, if they are no longer referenced anywhere in the DC
        // then the origin IMGAPI image should be deleted.
        var manifest;
        try {
        ...
        // Include a deletion message for the config layer.
        changes.push({ Deleted: manifest.config.digest });

        var digestChain = [];
        var hitImageHasDependentImagesError = false;
        var originData = manifest.layers.map(
            function (layer) {
                digestChain.push(layer.digest);
                return {
                    digest: layer.digest,
                    uuid: imgmanifest.imgUuidFromDockerDigests(digestChain)
                };
            }
        );

        vasync.forEachPipeline({
            func: deleteOriginImage,
            inputs: originData.slice(0, -1).reverse()
        }, cb);

        function deleteOriginImage(originDetails, next) {
            if (hitImageHasDependentImagesError) {
                next();
                return;
            }

            var digest = originDetails.digest;
            var origin_uuid = originDetails.uuid;

            // Ensure no other docker image uses this origin_uuid in the *DC*.
            var filter = [ { image_uuid: origin_uuid } ];
            Image.list(opts.app, log, filter, function (lookupErr, imgs) {
                if (lookupErr) {
                    next(lookupErr);
                    return;
                }
                ...
                if (imgs.length > 0) {
                    // This origin is used by another docker image.
                    hitImageHasDependentImagesError = true;
                    next();
                    return;
                }

                log.debug({origin_uuid: origin_uuid},
                    'deleteImage: delete imgapi image (origin)');
                app.imgapi.deleteImage(origin_uuid, {
                    headers: {'x-request-id': opts.req_id}
                }, function (err) {
                    if (err && err.restCode === 'ImageHasDependentImages') {
                        // This origin is used by another IMGAPI image.
                        hitImageHasDependentImagesError = true;
                        log.info({origin_uuid: origin_uuid},
                            'deleteImage: hit ImageHasDependentImages');
                        err = null;
                    } else if (!err) {
                        changes.push({ Deleted: digest });
                    }
                    next(err);
                });
            });
...
function createImage(opts, callback) {
    ...
    assert.arrayOfString(opts.payload.digestChain, 'opts.payload.digestChain');
    ...
    function createImgapiImage(ctx, next) {
        var imgOpts = {
            ...
            digestChain: opts.payload.digestChain,
            rat: rat,
            req: req
        };
    ...
    function createSdcDockerImageLayer(ctx, next) {
        var modelOpts = {
            ...
            index_name: rat.index.name,
            manifest_str: opts.payload.manifest_str,
            manifest_digest: opts.payload.manifest_digest,
            owner_uuid: account_uuid,
