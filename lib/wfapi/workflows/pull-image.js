...
function pullImageLayers(job, cb) {
    ...
    function processMessage(data, next) {
        if (data.type == 'error') {
            ...
            // 'head' is deprecated (used for v1 image pulls)
    ...
    function createDockerImage(data, next) {
        ...
        assert.optionalNumber(data.version, 'data.version');

        job.log.info('createDockerImage:: data: %s', JSON.stringify(data));

        var head = job.params.head;
        ...
        if (!data.version) {
            // This means an old imgapi is talking to us.
            assert.object(data.image, 'data.image');

            // Older v1 images are tied to the index_name.
            query.index_name = job.params.rat.index.name;
            image_uuid = data.image.uuid;

        } else if (data.version >= 2) {
            assert.string(data.head, 'data.head');
            assert.string(data.image_uuid, 'data.image_uuid');
            assert.string(data.manifest_str, 'data.manifest_str');
            assert.string(data.manifest_digest, 'data.manifest_digest');
            assert.number(data.size, 'data.size');

            head = data.head;
            image_uuid = data.image_uuid;
            manifest_str = data.manifest_str;
            manifest_digest = data.manifest_digest;
            size = data.size;

            job.params.head = head;  // needed for tagHeadImage
        }
        ...
        dockerAdmin.get({
            ...
            if (images.length) {
                ...
                var image = images[0];
                if (image.heads.indexOf(head) !== -1) {
                    next();
                    return;
                }

                action = 'update';
                image.heads.push(head);
                heads = image.heads;
            } else {
                action = 'create';
                heads = [ head ];
            ...
            var layer = {
                ...
                docker_id: imgId,
                docker_version: imgJson.docker_version,
                heads: heads,
            ...
            if (imgJson.history) {
                layer.history = imgJson.history;
            }
            if (imgJson.rootfs) {
                layer.rootfs = imgJson.rootfs;
            }
            if (imgJson.parent) {
